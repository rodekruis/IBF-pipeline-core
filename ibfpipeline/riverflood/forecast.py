...
from ibfpipeline.riverflood.data import (
    PipelineDataSets,
    ForecastDataUnit,
    FloodForecast,
...
class Forecast(PipelineModule):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            settings_to_check=["global_flood_maps_url"],
            secrets_to_check=[],
            **kwargs,
        )
    ...
    def __compute_triggers_station(self):
        ...
        for discharge_station in self.data.discharge_station.data_units:
            station_code = discharge_station._id
            lead_time = discharge_station.lead_time
            threshold_station = self.data.threshold_station.get_data_unit(
                _id=station_code
            )
            ...
            forecast_data_unit = ForecastStationDataUnit(
                _id=discharge_station._id,
                name=discharge_station.name,
